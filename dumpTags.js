...
/*const query =
  `select e.id from Fase f 
    ...
    group by e.id`*/
const query =
  `select distinct tema as tema from Papel
  union select distinct tema as tema from Observacion
  union select distinct tema as tema from Informe
  order by tema`

const inicio = new Date()
const tag = 'papelesTag'
...
mongo.client.connect().then(async () => {
  ...
  qy.on('row', data => {
    ...
    if (data.tema != "") param.options.push({ id: mongo.newId(), value: data.tema, color: '#C2C2C2' })
  })
  qy.on('done', () => {
    if (i) {
    ...
    mongo.db().collection('params').updateOne({ name: tag }, { $set: param },{upsert:true}).then(r => {
      console.log(r)
