...
const query =
  `select id,'Prórroga' as nombre, '20240316T15:00:00Z' as fecha, autorId, estudioId as padreId, 'project' as tipoPadre,
   dias, diasAprobados, descripcion, comentario
  from Prorroga`
...
function transform(o) {
  o.descripcion = `<strong>Solicitud de prórroga:</strong> <br />
  Días solicitados ${o.dias}, motivo:
  ${o.descripcion}<br/><br/>
  Días aprobados: ${o.diasAprobados}, comentario:<br/>
  ${o.comentario}`
  ++segundos
...
mongo.client.connect().then(async () => {
  ...
  qy.on('row', data => {
    ...
    docs.insert(doc)
    ids.insert({ _id: doc._id, table: 'prorroga', idSql: data.id })
    i += 1
    if (i > 10) {
      ids.execute()
